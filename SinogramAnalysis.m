...
M = importdata(file12);   %синограма из детектора
...
path = cd;
if J1 == 2.5
    Mref = importdata([path, '\Данные с детекторов\Референс\detData2018_10_18_cal_sin_jaw1new.csv']);
    Dref2 = importdata([path, '\Данные с детекторов\Референс\detData2018_10_18_cal_sin_jaw1_varsnew.csv']);
end
    
if J1 == 5.02
    Mref = importdata([path, '\Данные с детекторов\Референс\detData2018_10_18_cal_sin_jaw2.1new.csv']);
    Dref2 = importdata([path, '\Данные с детекторов\Референс\detData2018_10_18_cal_sin_jaw2.1_varsnew.csv']);
end
    
if J1 == 1.05
    Mref = importdata([path, '\Данные с детекторов\Референс\detData2018_10_18_cal_sin_jaw0.35new.csv']);
    Dref2 = importdata([path, '\Данные с детекторов\Референс\detData2018_10_18_cal_sin_jaw0.35_varsnew.csv']);
end

waitbar(.25,f,'Пожалуйста, подождите...');
pause(1)
                            
%% Данные с детектора

Mraw = M.data (1:(size(M.data,1)),30:559);
Mrefraw = Mref.data (:,30:559);  %без ненужных каналов

% Нормировка на референсное значение
Dref = Dref2.data(:,3);
D = D2.data(:,3);
SR = mean(D);   %Среднее значение дозы мониторной камеры
SRref = mean(Dref); %Среднее референсное значение дозы мониторной камеры
B = 5168.4; %Фоновое значение сигнала
...
waitbar(.50,f,'Пожалуйста, подождите...');
pause(1)
                            
% Проецируем на ось X
a1 = 0.064;  %Длина одной ячейки детектора
for a = 1:size(Mraw, 2)
    X(a) = 132*sin(a*a1/99)/(33/99+cos(a*a1/99));
end

% Интерполируем на 800 точек
d1 = 0.66;    %шаг интерполяции
...
size(Y)
% %Проверка на совпедение проекций
% for a = 1:size(Y, 2)
...
waitbar(.75,f,'Пожалуйста, подождите...');
pause(1)
...
%Построение синограммы с внешних детекторов
Y2 = 1:size(F11, 1);
d3 = 0.05;
X3= 0:d3:40;
Y1 = 1:size(F11, 2);   
...
colormap jet;
grid on; 
xlabel('Номер проекции');
ylabel ('X, см');
title('Данные со встроенных детекторов');
else

%% Загрузка файла DICOM RT
...
if isequal(char('bin'),D1)
    ...
    a = 0;
    for i=1:(S-1)
        B = ['A.BeamSequence.Item_1.ControlPointSequence.Item_' num2str(i) '.Private_300d_10a7']; % сохраняем имя файла
        C = eval(B);
        numOfRows = size(C, 1);
        if  numOfRows > a
            a = numOfRows;
    ...
    waitbar(.2,f,'Пожалуйста, подождите...');
    pause(1)
...
x0 = 0;
    ...
    % Сохраняем синограму в файле
    xlswrite(Fi,W);
    fclose('all'); 
    Tmlc = xlsread(Fi);
    delete(Fi); 
    numOfRows = size(Tmlc, 1); % число строк
    numOfCols = size(Tmlc, 2); % число столбцов
    Tmlc(isnan(Tmlc))=0;
    J = A.BeamSequence.Item_1.ControlPointSequence.Item_1.BeamLimitingDevicePositionSequence.Item_2.LeafJawPositions;
...
Tmlc';
waitbar(.4,f,'Пожалуйста, подождите...');
pause(1)
...
set(handles.text22, 'String',str3);
if J1 == 5.02
% Учет времени открытия самого лепестка и двух соседних
for p = 1:size(Tmlc, 1)
...
%Учет количества открытых соседних лепестков
for p = 1:size(Tmlc, 1)
    for i = 2:(size(Tmlc, 2)-1)
        h1=0;
        h2=0;
...
size(Tmlc);
size(Tmlc);
b = 0.35;    %ширина полутени
L = 0.625;  %длина лепестка в изоцентре
F = zeros(size(Tmlc, 1),size(Tmlc, 2));
i = 0;
d2 = 0.05;  %Шаг интерполяции
...
% F(217,209)
% F(217,222)
waitbar(.6,f,'Пожалуйста, подождите...');
pause(1)
...
%Интерполяция на 800 точек референс
d1 = 0.66;    %шаг интерполяции
for p = 1:size(Mr2, 1)
    for a = 1:size(Mr2, 2)
        i = 0;
...
waitbar(.8, f,'Пожалуйста, подождите...');
pause(1)

%Переворачиваем матрицу с СП
% for p = 1:size(Doze, 1)
%     for i = 1:size(Doze, 2) 
...
% Учет рассеивания на концах поля
F11 = F1;
% Слева
...
handles.dif2 = dif2;
guidata(gcbo, handles);   
%Построение синограммы с системы планирования 
X3= 0:d2:40;
Y2 = 1:size(F11, 1);
% x0 = 0;
if l == 0
...
xlabel('Номер проекции');
ylabel ('X, см');
title('Данные с системы планирования');
end 
...
%Построение второго набора данных с внешних детекторов
Y2 = 1:size(A1, 1);
d3 = 0.05;
X3= 0:d3:40;
Y1 = 1:size(A1, 2);   
...
%Сохранение матриц A1, A2
handles.data1 = A1;
guidata(gcbo, handles);
handles.data2 = A2;
guidata(gcbo, handles);

%Одномерный гамма-индекс
v = get(handles.checkbox4,'Value');
if v == 1 
    DTA = 3;
    dosed = 0.03;
...
grid on;
axis([1 64 0 100]);
mylegend  = ['Гамма-индекс по лепесткам'];
xlabel('Номер лепестка');
ylabel('Гамма-индекс, %'); 
title(mylegend);
...
%Создание слайдера для различных проекций
...
v = get(handles.checkbox4,'Value');
% if v == 0 
%Построение двух распределений на выбранной проекции
axes(handles.axes6);
size(A11,1);
size(A11,2);
Y6 = A11(p,1:801);
...
if v == 1
xlabel('X, см');
ylabel ('Время открытия лепестка, %');
legend('Сигнал с детекторов 1','Сигнал с детекторов 2')  ; 
else
xlabel('X, см');
ylabel('Время открытия лепестка, %'); 
legend('Расчетные данные','Сигнал с детекторов')  ;
end
...
%% Выполнение двумерного гамма-анализа
function pushbutton4_Callback(hObject, eventdata, handles)

% Данные о диапазоне расчета гамма-индекса
p1 = handles.datap1;
p2 = handles.datap2;
x1 = handles.datax1;
x2 = handles.datax2;
...
% Построение гистограммы для гамма-индекса
axes(handles.axes5);
nbins = 100;
Zmax = max(max(Z));
xbins = [0 Zmax/2];
...
xlabel('Гамма-индекс') 
ylabel ('Вероятность, %')
title('Гистограмма гамма-индекса')

% Расчет значения гаммы
n1 = 0;
n2 = 0;
n3 = 0;
for p = 1:size(A1,2)
...
% Слайдер анализа движения каждого лепестка
...
% Погрешность для значения потока энергии
...
%% Выполнение одномерного гамма-анализа
...
max(max(A11));
D = mean(mean(A33));
myString5 = ['Средняя ошибка = ', num2str(D)];
set(handles.text21, 'String', myString5);
...
myString2 = ['Число точек, прошедших критерий = ', num2str(gamma3), '%'];  
myString3 = ['Полное количество точек = ', num2str(n2)];    
set(handles.text2, 'String', myString2);           
set(handles.text20, 'String', myString3);  
